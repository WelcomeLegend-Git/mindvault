...
object UserManager {
    ...
    fun init(context: Context) {
        // Restore calls while holding this monitor; ordinary repeated Activity init is a no-op.
        if (initialized && !Thread.holdsLock(this)) return
        initialize(context)
    }

    @Synchronized
    private fun initialize(context: Context) {
        if (!ManagerPersistence.writable() && ::prefs.isInitialized) return
    ...
    fun createUser(
        ...
        if (!ManagerPersistence.writable()) return null
    ...
    fun logoutUser() {
        if (!ManagerPersistence.writable()) {
            // The durable pending-local-logout gate is retired by AuthManager only after disk
            // cleanup succeeds. Quarantine must still clear observable authentication now.
            _currentUser.value = null
            _isLoggedIn.value = false
    ...
    fun updateCurrentUser(
        ...
        if (!ManagerPersistence.writable()) return false
    ...
    private fun saveUser(user: User): Boolean {
        return try {
            ...
            editor.putString(
                "user_${user.id}_last_active",
                user.lastActiveAt.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
            )
            ...
            // Add to user list
            val userIds = prefs.getStringSet("all_user_ids", emptySet()).orEmpty().toMutableSet()
            userIds.add(user.id)
    ...
    private fun createSession(userId: String) {
        ...
        prefs.edit()
            ...
            .putString(
                "session_${sessionToken}_expires",
                session.expiresAt.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
            )
            .apply()
    }

    private fun invalidateUserSessions(userId: String) {
        // Capture the sessions to delete BEFORE filtering, otherwise their persisted
        // records are never removed.
        val sessionsToDelete = _userSessions.value.filter { it.userId == userId }
        _userSessions.value = _userSessions.value.filter { it.userId != userId }

        val editor = prefs.edit()
        // Sessions from previous processes are not present in the in-memory flow.
        prefs.all.filter { (key, value) ->
            key.startsWith("session_") && key.endsWith("_user") && value == userId
        }.keys.forEach { key ->
            val prefix = key.removeSuffix("_user")
            editor.remove(key).remove("${prefix}_login").remove("${prefix}_expires")
        }
